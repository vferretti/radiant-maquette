...
- **Panneaux de détail groupés + dump brut, avec sélecteur de membre** (`.ov-memsel` [Proband | Mère | Père], variable `OV_MEMBER`) : la section détail reflète le membre choisi (colonne correspondante surlignée dans le tableau trio). Ordre des panneaux (gauche→droite) : **Alignement · Couverture · Enrichissement · Sexe · Variants · CNV · Séquençage (run)**. Métriques formatées ; verdicts inline uniquement pour les seuils définis par clin (duplication/fold-80 en valeur brute dans le détail). Catalogue `OV_PANELS` (+ carte « Séquençage (run) » construite à part depuis `SEQ_QC` : run/flowcell/séquenceur/date/kit). Le titre ne porte plus la ligne de contexte du run (déplacée dans la carte le 2026-07-13). Pas de `BAIT_SET` (doublon du kit). Pas de sous-titre explicatif sous « Vue d'ensemble » (une page Notion documentera l'ensemble plus tard).
